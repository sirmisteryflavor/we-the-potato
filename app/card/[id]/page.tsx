...
import type { CardTemplate, VoterCardData, VoterCardDecision } from "@/lib/schema";
...
export default function FinalCardPage() {
  ...
  const { data: card, isLoading } = useQuery<FinalizedVoterCard>({
    ...
    queryFn: async () => {
      const url = `/api/finalized-card/${cardId}`;
  ...
  const handleDownload = async (aspectRatio: "9:16" | "1:1" = "9:16") => {
    ...
    try {
      ...
      const shareUrl = getShareUrl(cardId || '', undefined, card?.shareUrl);
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <AppHeader
        showBackButton
        onBack={() => router.push(getBackDestination(!!isOwner))}
