...
import type { MeasureDecision, Candidate, BallotMeasure } from "@/lib/schema";
                ...
                {stats.totalDecided === stats.totalItems && stats.totalItems > 0 ? (
                  <>
                    ...
                    <Button
                      onClick={() => {
                        if (existingCard) {
                          router.push(`/card/${existingCard.id}?from=ballot${eventId ? `&eventId=${eventId}` : ""}`);
                        } else {
                          router.push(eventId ? `/voter-card?eventId=${eventId}` : "/voter-card");
                        }
                      }}
